...
function speakSystem(text, onEnd, rate = 0.75) {
  if (!('speechSynthesis' in window)) { onEnd?.(); return; }
  speechSynthesis.cancel();
  const u = new SpeechSynthesisUtterance(text);
  u.lang = 'ja-JP';
  u.rate = normalizeRate(rate);
...
async function speakEleven(text, onEnd, voiceId, rate) {
  const s = getSettings();
  try {
    const url = await fetchElevenAudio(text, s, voiceId);
    stopSpeaking();
    currentAudio = new Audio(url);
    currentAudio.playbackRate = normalizeRate(rate);
    currentAudio.defaultPlaybackRate = normalizeRate(rate);
    ...
    speakSystem(text, onEnd, rate);
  }
}

// ---------- 对外接口 ----------
// voiceId：ElevenLabs 音色（用于 A/B 角色区分）；系统语音下忽略
export function speak(text, onEnd, voiceId, rate) {
  const s = getSettings();
  const spokenText = cleanTTSText(text);
  const playbackRate = normalizeRate(rate ?? s.ttsRate);
  if (s.ttsProvider === 'elevenlabs' && s.elevenKey) {
    speakEleven(spokenText, onEnd, voiceId, playbackRate);
  } else {
    speakSystem(spokenText, onEnd, playbackRate);
  }
}
